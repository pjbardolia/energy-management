...
# rather than the generic read_modbus()/VFD_REGISTER_MAPS path.
_NON_VFD_MODELS = {"WAVESHARE_AI8CH"}
...
def load_config() -> dict:
    ...
    for b_idx, bus in enumerate(buses):
        ...
        for d_idx, device in enumerate(devices):
            ...
            if device["vfd_model"] not in VFD_REGISTER_MAPS and device["vfd_model"] not in _NON_VFD_MODELS:
                raise ValueError(
...
def read_ai8ch_pressure(client: ModbusSerialClient, slave_id: int) -> dict | None:
    ...
    return {"pressure": round(pressure_kgcm2, 3)}
# ---------------------------------------------------------------------------
